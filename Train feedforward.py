...
lyr_in = Input(shape=(10,))
lyr = Dense(units=64, activation='relu', kernel_initializer=initializer)(lyr_in)
lyr = Dense(units=32, activation='relu', kernel_initializer=initializer)(lyr)
lyr_out = Dense(units=1, activation='linear', kernel_initializer=initializer)(lyr)

model = Model(lyr_in, lyr_out)
...
result = model.fit(X_train, Y_train, epochs=250, batch_size=256, verbose=1, callbacks=[earlystopping])
